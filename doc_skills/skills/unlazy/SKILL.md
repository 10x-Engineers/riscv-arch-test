...
name: unlazy
description: >-
  Default completion discipline for substantial Cursor/Codex agent work in this
  repo. Write acceptance gates before execution, decompose with the Depth Tree,
  run approved checks, and re-verify evidence before reporting. Use for long or
  multi-part tasks, half-done returns, exhaustive audits/builds, parallel leaves
  or pipelines, or explicit triggers such as /unlazy, $unlazy, "tree N",
  "gates", and "do not stop until it is done". Skip only for trivial edits or
