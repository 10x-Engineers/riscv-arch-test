...
_MHSELECT_MCONTEXT = 4

_TEXTRA_SSELECT_OFFSET = 0
_TEXTRA_SVALUE_OFFSET = 2
_TEXTRA32_SVALUE_WIDTH = 16  # bits 17:2, below the RV32 sbytemask field at bit 18
_TEXTRA64_SVALUE_WIDTH = 34  # bits 35:2, below the RV64 sbytemask field at bit 36
_SSELECT_SCONTEXT = 1

_DOUBLE_CONTEXT_LIVE_MCONTEXT_RV32 = 0b111111
_DOUBLE_CONTEXT_LIVE_MCONTEXT_RV64 = 0x1FFF
_DOUBLE_CONTEXT_LIVE_SCONTEXT_RV32 = 0x0000FFFF
_DOUBLE_CONTEXT_LIVE_SCONTEXT_RV64 = 0xFFFFFFFF

_DOUBLE_CONTEXT_MHVALUE_CASES = (
    ("mismatch", 0b000000, 0x0000),
    ("match", 0b111111, 0x1FFF),
)
_DOUBLE_CONTEXT_SVALUE_CASES = (
    ("mismatch", 0x0000, 0x00000000),
    ("match", 0xFFFF, 0xFFFFFFFF),
)
...
def _check_textra_result(reg: int, temp_reg: int, mode: str, test_data: TestData, expect_fire: bool) -> list[str]:
    """Sigupd raw a1/mcause state, plus a derived match flag: 0 iff a1's fired/not-fired
    state actually equals expect_fire.
    """
    cause_csr = "mcause" if mode == "Sm" else "scause"
    expected_a1 = 0 if expect_fire else 1
    return [
        f"mv x{reg}, a1 # 0 iff fired, 1 iff untouched",
        write_sigupd(reg, test_data),
        f"xori x{temp_reg}, x{reg}, {expected_a1} # 0 iff actual outcome matches expected ({'fire' if expect_fire else 'no-fire'})",
        write_sigupd(temp_reg, test_data),
        f"csrr x{reg}, {cause_csr} # informational only",
        write_sigupd(reg, test_data),
    ]


def _textra3_double_context(mhselect: int, mhvalue: int, sselect: int, svalue: int, xlen: int) -> int:
    """tdata3 combining mhselect/mhvalue with sselect/svalue; sbytemask stays 0."""
    if xlen == 64:
        return (
            (mhselect << _TEXTRA64_MHSELECT_OFFSET)
            | ((mhvalue & ((1 << _TEXTRA64_MHVALUE_WIDTH) - 1)) << _TEXTRA64_MHVALUE_OFFSET)
            | ((svalue & ((1 << _TEXTRA64_SVALUE_WIDTH) - 1)) << _TEXTRA_SVALUE_OFFSET)
            | (sselect << _TEXTRA_SSELECT_OFFSET)
        )
    return (
        (mhselect << _TEXTRA32_MHSELECT_OFFSET)
        | ((mhvalue & ((1 << _TEXTRA32_MHVALUE_WIDTH) - 1)) << _TEXTRA32_MHVALUE_OFFSET)
        | ((svalue & ((1 << _TEXTRA32_SVALUE_WIDTH) - 1)) << _TEXTRA_SVALUE_OFFSET)
        | (sselect << _TEXTRA_SSELECT_OFFSET)
    )


def _load_textra_double_context_tdata3(
    reg: int, mhselect: int, mhvalue_rv32: int, mhvalue_rv64: int, sselect: int, svalue_rv32: int, svalue_rv64: int
) -> list[str]:
    """XLEN-guarded LI of tdata3 combining mhselect/mhvalue and sselect/svalue."""
    return [
        "#if __riscv_xlen == 64",
        f"LI(x{reg}, 0x{_textra3_double_context(mhselect, mhvalue_rv64, sselect, svalue_rv64, 64):x})",
        "#else",
        f"LI(x{reg}, 0x{_textra3_double_context(mhselect, mhvalue_rv32, sselect, svalue_rv32, 32):x})",
        "#endif",
    ]


def _load_double_context_live_mcontext(reg: int) -> list[str]:
    """XLEN-guarded LI of the double-context coverpoint's fixed mcontext test pattern."""
    return [
        "#if __riscv_xlen == 64",
        f"LI(x{reg}, 0x{_DOUBLE_CONTEXT_LIVE_MCONTEXT_RV64:x})",
        "#else",
        f"LI(x{reg}, 0x{_DOUBLE_CONTEXT_LIVE_MCONTEXT_RV32:x})",
        "#endif",
    ]


def _load_double_context_live_scontext(reg: int) -> list[str]:
    """XLEN-guarded LI of the double-context coverpoint's fixed scontext test pattern."""
    return [
        "#if __riscv_xlen == 64",
        f"LI(x{reg}, 0x{_DOUBLE_CONTEXT_LIVE_SCONTEXT_RV64:x})",
        "#else",
...
def _generate_textra_tests(test_data: TestData, mode: str) -> list[TestChunk]:
    ...
    ######################################
    coverpoint = "cp_sdtrig_textra_double_context"
    ######################################
    ...
    lines.append(
        comment_banner(
            coverpoint,
            "textra mhselect/mhvalue (mcontext) and sselect/svalue (scontext) enabled\n"
            "together: fires only when every enabled context number matches",
        )
    )

    dc_cfg_reg, dc_addr_reg, dc_data_reg, dc_temp_reg = test_data.int_regs.get_registers(
    ...
    for trig_num in range(UDB_NUM_TRIGGERS):
        ...
        lines.append(f"\n# trig_num {trig_num}")
        lines.extend(_load_double_context_live_mcontext(dc_temp_reg))
        lines.append(_csr_access(f"csrw mcontext, x{dc_temp_reg} # live context tag", mode))
        lines.extend(_load_double_context_live_scontext(dc_temp_reg))
        lines.append(_csr_access(f"csrw scontext, x{dc_temp_reg} # live context tag", mode))

        for mhselect in (0, _MHSELECT_MCONTEXT):
            for sselect in (0, _SSELECT_SCONTEXT):
                for mhvalue_case, mhvalue_rv32, mhvalue_rv64 in _DOUBLE_CONTEXT_MHVALUE_CASES:
                    for svalue_case, svalue_rv32, svalue_rv64 in _DOUBLE_CONTEXT_SVALUE_CASES:
                        ...
                        for trig_type in ("icount", "itrigger", "etrigger", "mcontrol6"):
                            ...
                            lines.extend(guards)
                            binname = (
                                f"trig_num_{trig_num}_type_{trig_type}"
                                f"_mhsel{mhselect}_mhvalue_{mhvalue_case}"
                                f"_ssel{sselect}_svalue_{svalue_case}"
                            )
                            lines.extend(
                                [
                                    _add_tc(test_data, binname, coverpoint, covergroup),
                                    *_config_textra_trigger(
                                        dc_cfg_reg,
                                        trig_num,
                                        trig_type,
                                        _load_textra_double_context_tdata3(
                                            dc_cfg_reg,
                                            mhselect,
                                            mhvalue_rv32,
                                            mhvalue_rv64,
    ...
    test_data.int_regs.return_registers([dc_cfg_reg, dc_addr_reg, dc_data_reg, dc_temp_reg])
