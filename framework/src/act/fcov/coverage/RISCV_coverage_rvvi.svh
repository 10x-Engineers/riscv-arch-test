...
function void save_rvvi_data(bit trap, int hart, int issue, string disass);
  ...
  // Copy RVVI mem-access into ins.current so covergroups can sample .pte / .gpte
  rvviData.mem_i = this.rvvi.mem_i[hart][issue];
  rvviData.mem_d = this.rvvi.mem_d[hart][issue];
