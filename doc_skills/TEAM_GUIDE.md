...
**Plug-in:** this folder — [`PLUGIN.md`](PLUGIN.md) · [`AGENTS.md`](AGENTS.md) · `./install.sh link /path/to/ACT4`

| Doc in this pack | Role |
|------------------|------|
| [`HOW_TO_USE.md`](HOW_TO_USE.md) | 2-minute skill picker |
| [`EXTERNAL_LINKS.md`](EXTERNAL_LINKS.md) | norm-rules URL, sheets, tools |
| [`PATH_INDEX.md`](PATH_INDEX.md) | Full path catalog (not repeated below) |
| [`TEAM_GUIDE.md`](TEAM_GUIDE.md) | This guide |

**With a full ACT4 checkout also see:** `handoffs/AI_TOOLING.md` · `handoffs/ROOT-AGENTS.md`.
...
| Share with a teammate? | Copy **this whole folder** or `./install.sh link /path/to/ACT4` |
...
| **Implement** | Write generators / docs | Add `generate_foo`, register `_emit(...)` in suite entry |
...
```text
  ...
  │  Register explicit _emit(...) per test; reuse *Common.py           │
...
| Python generators | explicit `_emit(...)` × 39 in `SvH.py` (full repo) |
...
| Layer | One line |
|-------|----------|
| Pack | `./install.sh link …` or read `AGENTS.md` + one skill |
